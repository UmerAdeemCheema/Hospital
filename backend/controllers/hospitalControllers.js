...
const upateHospital = asyncHandler(async (req, res) => {
  try {
    const hospital = await Hospitals.findByIdAndUpdate(req.params.id, {
      $set: {hospitalDescription: req.body.description},
    });
    res.status(200).json(hospital);
  } catch (error) {
    res.status(400);
...
const addDoctors = asyncHandler(async (req, res) => {
  try {
    ...
    if (!req.body.name || !req.body.doctorId) {
      console.log("aa")
      res.status(400);
      throw new Error("ALL FIELDS REQUIRED");
    }
...
const hospitalEventDetails = asyncHandler(async (req, res) => {
  try {
    const hospital = await Hospitals.findById(req.body.id);
    res.status(200).json(hospital);
  } catch (e) {
    res.status(400);
    throw new Error(error);
