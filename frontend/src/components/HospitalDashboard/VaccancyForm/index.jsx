...
const VaccancyForm = ({ open, setOpen, vacancyCards, setVacancyCards }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    let reqBody = {
      method: "post",
      url: `http://localhost:5000/api/hospitals/${id}/addVaccancy`,
      data: { position, amount, desc },
    };
    let [data, error] = await axiosRequest(reqBody);
    setIsSubmitting(false);
    if (data) {
      alert("Successfully added vaccancy");
      var abc = vacancyCards;

      abc.vaccancy.push(reqBody.data);
